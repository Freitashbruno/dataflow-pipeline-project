...
class AddWindowdtlsFn(beam.DoFn):
    def process(self, element, window=beam.DoFn.WindowParam):
        ...
        element['window_start'] = window_start.isoformat()
        element['window_end'] = window_end.isoformat()
        return [element]
...
def run(input_file, output_path, pipeline_args=None):
    ...
    with beam.Pipeline(options=pipeline_options) as p:
        ...
        timestamped_events = (
            non_none_events
            | 'Events with Timestamps' >> beam.Map(lambda events: beam.window.TimestampedValue(events, events['event_time'].timestamp()))
            | 'Log Timestamped Events' >> beam.Map(lambda x: logging.info(f"Timestamped Event: {x}") or x)
        )

        final_results = (
            timestamped_events
            | 'Events fixed Window' >> beam.WindowInto(window.FixedWindows(5 * 60))  # 5 minutes window
            | 'Add Window Info' >> beam.ParDo(AddWindowdtlsFn())
            | 'Log Final Results' >> beam.Map(lambda x: logging.info(f"Final Result: {x}") or x)
        )
        ...
        (
            ...
            | 'Write to BigQuery' >> beam.io.WriteToBigQuery(
                ...
                dataset='gdab',
                project='gdab-430616',
                schema='event_nbr:STRING, event_time:TIMESTAMP, window_start:TIMESTAMP, window_end:TIMESTAMP',
                create_disposition=beam.io.BigQueryDisposition.CREATE_IF_NEEDED,
                write_disposition=beam.io.BigQueryDisposition.WRITE_APPEND,
                custom_gcs_temp_location='gs://streamdata_bk/temp'
            )
        )
